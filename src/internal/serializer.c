...
static cco_error_t serialize_string(const char* str, size_t len, cco_serializer_context_t* ctx)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        switch (c) {
            ...
            case '\t': err = cco_strbuf_append(ctx->out, "\\t", 2); break;
            case '\r': err = cco_strbuf_append(ctx->out, "\\r", 2); break;
            case '\\': err = cco_strbuf_append(ctx->out, "\\\\", 2); break;
            case '"':  err = cco_strbuf_append(ctx->out, "\\\"", 2); break;
            default:
                if (c >= 0 && c < 32) {
...
static cco_error_t serialize_recursive(const cco_object_t* obj, cco_serializer_context_t* ctx)
{
    if (!obj) return CCO_ERR_INVALID_ARG;
    ...
    switch (obj->type) {
        ...
        case CCO_TYPE_BOOLEAN:
            if (obj->as.boolean) err = cco_strbuf_append(ctx->out, "true", 4);
            else err = cco_strbuf_append(ctx->out, "false", 5);
            break;
        ...
        case CCO_TYPE_ARRAY:
            ...
            break;
        case CCO_TYPE_MAP:
            /* Omitted full map implementation for brevity, similar to array but with keys */
            err = cco_strbuf_append(ctx->out, "()", 2);
            break;
        case CCO_TYPE_TEMPLATE_INSTANCE:
            err = cco_strbuf_append(ctx->out, "#", 1);
            if (err == CCO_OK && obj->as.tmpl_inst.template_name) {
                err = cco_strbuf_append(ctx->out, obj->as.tmpl_inst.template_name, strlen(obj->as.tmpl_inst.template_name));
            }
            if (err == CCO_OK) {
...
cco_error_t cco_serialize(const cco_object_t* obj, cco_strbuf_t* out_buf, bool pretty)
{
    if (!obj || !out_buf) return CCO_ERR_INVALID_ARG;
