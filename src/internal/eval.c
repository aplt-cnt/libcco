                                ...
                                cco_object_t* expr_ast, cco_object_t** out_val)
{
    ...
    (void)expr_ast;
    (void)out_val;
    cco_diag_record(CCO_ERR_FORBIDDEN, 0, 0, "cco_eval_expression",
    ...
    {
        cco_diag_record(CCO_ERR_FORBIDDEN, 0, 0, "cco_eval_expression",
                        "Expressions are disabled by options");
        return CCO_ERR_FORBIDDEN;
    }
                                    ...
                                    cco_object_t** out_val)
{
    ...
    (void)format_str;
    (void)out_val;
    cco_diag_record(CCO_ERR_FORBIDDEN, 0, 0, "cco_eval_builtin_format",
                    "$format is disabled");
    return CCO_ERR_FORBIDDEN;
#else
                                 ...
                                 const char* env_name, cco_object_t** out_val)
{
    ...
    (void)env_name;
    (void)out_val;
    cco_diag_record(CCO_ERR_FORBIDDEN, 0, 0, "cco_eval_builtin_env",
                    "$env is disabled");
    return CCO_ERR_FORBIDDEN;
#else
