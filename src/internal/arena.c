...
static cco_arena_block_t* cco_arena_alloc_block(size_t min_capacity)
{
    ...
    cco_arena_block_t* block = (cco_arena_block_t*)malloc(sizeof(cco_arena_block_t) + capacity);
    if (block) {
        block->next = NULL;
...
void* cco_arena_alloc(cco_arena_t* restrict a, size_t n)
{
    ...
    /* Check total limit */
    if (a->max_allocation_limit > 0 && SIZE_MAX - a->total_allocated < aligned_n) {
        return NULL;
    }
    if (a->max_allocation_limit > 0 && a->total_allocated + aligned_n > a->max_allocation_limit) {
