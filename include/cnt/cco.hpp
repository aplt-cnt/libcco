 ...
 * According to CPS Section 5.5, C++ exceptions are forbidden in internal engineering code.
 * However, this is a thin public C++ wrapper layer over the C API designed for external consumers.
 * Therefore, exception mapping is exempt and explicitly allowed here.
 */
...
public:
    ...
    std::string serialize(bool pretty = false) const {
        ...
        free(cstr); // Important: cco_serialize_to_string requires caller to free
        return result;
    }
...
inline Object parse(const std::string& src, const cco_parse_options_t* opts = nullptr) {
