...
int main(void)
{
    ...
    cco_object_t* config =
        cco_parse_string(mixed_text, strlen(mixed_text), &opts);
    ...
    if (config)
    {
        ...
        if (err == CCO_OK)
        {
            printf("A warning was recorded by the parser to indicate "
                   "auto-correction.\n");
        }
